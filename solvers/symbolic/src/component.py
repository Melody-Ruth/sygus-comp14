...
class Component:
    ...
    def SetInputLabels(self, labels):
        if len(labels) != len(self.inputPorts):
            raise SynthException('len(Labels) != len(self.InputPorts) in' +
                                 'Component.SetInputLabels')
        self.inputLabels = labels
        self.inputLabelMap = {self.inputPorts[i].name: labels[i]
    ...
    def PrintExpr(self, args):
        if isinstance(self.production, list):
            retVal = '(synth-fun %s' % self.production[0]
            for arg in args:
                retVal += ' '
                retVal += arg
            retVal += ')'
